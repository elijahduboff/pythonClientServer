...
def listen_message_from_server(server):
    while True:
...
def send_message_to_chat(server):
    while True:
...
def main():
    ...
    receiver = threading.Thread(target=listen_message_from_server, args=(server,))
    receiver.daemon = True
    receiver.start()
    # receiver.join()
    sender = threading.Thread(target=send_message_to_chat, args=(server,))
    sender.daemon = True
    sender.start()
    sender.join()
